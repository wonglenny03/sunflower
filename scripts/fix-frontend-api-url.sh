...
# 创建/更新 .env.production
log_info "创建/更新 .env.production 文件..."
...
if [ -f "ecosystem.config.js" ]; then
    ...
    sed -i "s|NEXT_PUBLIC_API_URL: process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'|NEXT_PUBLIC_API_URL: 'http://${PUBLIC_IP}:3001'|g" ecosystem.config.js || true
    
    log_success "ecosystem.config.js 已更新"
else
    log_warning "ecosystem.config.js 不存在"
fi

# 重新构建前端（重要！环境变量在构建时使用）
log_info "重新构建前端（这可能需要几分钟）..."
log_warning "注意：NEXT_PUBLIC_API_URL 环境变量在构建时注入，必须重新构建！"
...
if command -v pm2 &> /dev/null; then
    ...
    log_warning "PM2 未安装，请手动重启前端应用"
fi

echo ""
echo "============================================"
log_success "前端 API URL 配置完成！"
echo "============================================"
echo ""
echo -e "${CYAN}配置信息:${NC}"
echo "  前端 URL: http://${PUBLIC_IP}:3000"
echo "  后端 API URL: http://${PUBLIC_IP}:3001/api"
echo "  Swagger 文档: http://${PUBLIC_IP}:3001/api/docs"
echo ""
echo -e "${CYAN}重要提醒:${NC}"
